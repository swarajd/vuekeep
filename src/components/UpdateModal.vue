<template>
    <div v-if="note" transition="modal" class="backdrop" v-on:click="dismissModal">
        <form class="edit-note" v-on:submit.prevent="updateThis" v-on:click.stop="">
            <input name="title" v-model="note.title" placeholder="Title" />
            <textarea name="content" v-model="note.text" placeholder="Text goes here..." rows="8"></textarea>
            <button type="button" v-on:click="deleteThis()">
...
<script>
import { deleteNote, updateNote } from '../vuex/actions';
